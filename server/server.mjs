...
export function createApp({store,homes,token,passwordHash,origins=['https://kartikkp.github.io']}) {
  ...
  const auth=passwordHash?passwordAuth({passwordHash,secret:token}):null;
  ...
  return http.createServer(async(req,res)=>{
    try {
      ...
      if(path==='/login'&&req.method==='POST'){
        if(!auth)return respond(res,503,{error:'Password login is not configured yet.'});
        if(!req.headers['content-type']?.startsWith('application/json'))return respond(res,415,{error:'JSON required'});
        let raw='',length=0;for await(const part of req){length+=part.length;if(length>1024)return respond(res,413,{error:'Login request too large'});raw+=part;}
        let body;try{body=JSON.parse(raw);}catch{return respond(res,400,{error:'Invalid JSON'});}
        const result=await auth.login(body?.password,ip);if(result.status===429)res.setHeader('Retry-After','900');
        const {status,...data}=result;return respond(res,status,data);
      }
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href) {
  ...
  const server=createApp({store,homes,token:process.env.HOUSEHOLD_TOKEN,passwordHash:process.env.LOGIN_PASSWORD_HASH,origins:(process.env.ALLOWED_ORIGINS||'https://kartikkp.github.io').split(',')});
